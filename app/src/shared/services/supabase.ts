...
export function readSupabaseConfig(env?: SupabaseEnv): SupabaseConfig {
  const url = (
    env
      ? env[SUPABASE_ENV_KEYS.url]
      : process.env.EXPO_PUBLIC_SUPABASE_URL ?? process.env[SUPABASE_ENV_KEYS.url]
  )?.trim();
  const publishableKey = (
    env
      ? env[SUPABASE_ENV_KEYS.publishableKey]
      : process.env.EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY ??
        process.env[SUPABASE_ENV_KEYS.publishableKey]
  )?.trim();
